...
type row_ty                  is table of point_ty index by pls_integer;
type pentomino_ty            is table of row_ty   index by pls_integer;
...
type v_pentomino_ty          is record (x number(1), y number(1), color char(1));
...
is
l_bool   boolean;
l_pentomino_rows pentomino_pkg.v_pentomino_row_ty;
l_x      number(2);
l_y      number(2);
begin
for j in 1 .. pentomino_pkg.p_stones
loop
  ...
  then
    ...
    loop
      l_bool := true;
      select x, y, color bulk collect into l_pentomino_rows from v_pentomino where series = j and orientation = o.orientation;
      <<done1>>
      for k in 1 .. l_pentomino_rows.count
      loop
        l_bool := p_x + l_pentomino_rows(k).x between 1 and pentomino_pkg.p_width and p_y + l_pentomino_rows(k).y between 1 and pentomino_pkg.p_heigth;
 	...
 	<<ready>>
        ...
        if    l_y > pentomino_pkg.p_heigth
        then  save_pentomino;
        elsif not l_bool
        then  resolve_pentomino(l_x, l_y);
        end if;

        for m in 1 .. l_pentomino_rows.count
        loop
          p_pentomino(p_x + l_pentomino_rows(m).x)(p_y + l_pentomino_rows(m).y).field := pentomino_pkg.free;
        end loop;
        pentomino_pkg.p_stone(j).available := true;
       end if;
     end loop;
   end if;
   end loop;
